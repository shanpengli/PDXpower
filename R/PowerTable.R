...
PowerTable <- function(n, m, beta, tau2 = 0.5, alpha = 0.05, lambda = 0.03,
  ...
  SumPowertable <- NULL
  for (j in 1:length(n)) {
    Powertable <- matrix(NA, nrow = length(m), ncol = 5)
    for (i in 1:length(m)) {
      Model1 <- vector()
      Model2 <- vector()
      censor.rate <- vector()
      if (distr == "Weibull") {
        a <- simfit(sim = sim, n = n[j], m = m[i], beta = beta, tau2 = tau2,
                    ...
                    lambdaC = lambdaC, censor = censor, model = print, ncores = ncores)
      ...
      if (print == "both") {
        ...
        Powertable[i, 5] <- mean(censor.rate)*100
    ...
    colnames(Powertable) <- c("NofLine", "NofMice", "ANCOVArandom", "Coxrandom", "censoringrate")
